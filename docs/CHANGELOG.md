...
**Fix per DB grandi: lock nel backfill node_uuid, scheda US PDF con centinaia di rapporti, Harris matrix oltre 600 archi** — pyarchinit tag `large-relations-5.13.12-alpha` (ramo `Stratigraph_00001`, commit `8ef4347c`); i fix PDF e matrix sono usciti anche su `master` come **4.9.11** (tag `v4.9.11`, commit `26adb90b`). Tutto emerso su un DB SQLite da 11 MB con 1311 US e due hub con ~545 rapporti ciascuno.

- **`scripts/migrations/_2026_05_node_uuid_backfill_lib.py`**: `backfill_uuids()` faceva la reflection delle PK con `inspect(handle.engine)` (seconda connessione del pool) dentro `engine.begin()`; superata la page-cache SQLite il writer saliva a lock EXCLUSIVE e la reflection attendeva la stessa transazione → `OperationalError: database is locked`. Ora `inspect(conn)` sulla connessione della transazione; `add_columns()` chiama `_columns_of()` prima di aprire la transazione. Test `tests/migrations/test_node_uuid_backfill.py::test_backfill_survives_page_cache_spill` (`PRAGMA cache_size=1`).
- **`modules/utility/pyarchinit_exp_USsheet_pdf.py`**: nuove costanti `MAX_INLINE_RAPPORTI = 80`, `MAX_INLINE_RAPPORTI_INDEX = 40`, `ANNEX_TARGETS_PER_ROW = 60`, dizionario `_RAPPORTI_I18N` (it/en/de/fr/es); helper `_cap_rapporti(obj, limit, more_fmt)`; metodi `single_US_pdf_sheet.cap_rapporti_for_sheet(lang)` (chiamato dopo `unzip_rapporti_stratigrafici*()` in `create_sheet_archeo3_usm_fields_2` / `create_sheet_en` / `create_sheet_de`) e `single_US_pdf_sheet.create_rapporti_annex(lang) -> list[Flowable]` (allegato con una riga ogni 60 US, `[]` se nulla è stato troncato); tutti i `build_US_sheets*` accodano l'allegato dopo la scheda; `US_index_pdf_sheet.getTable*` limitano a 40. Risolve `reportlab.platypus.doctemplate.LayoutError: Flowable ... too large on page` (blocco righe 13-22 bloccato dagli SPAN; `Table(splitInRow=1)` scartato: una riga per pagina). Test `tests/utility/test_us_pdf_rapporti_overflow.py` (11).
- **Nuovo modulo `modules/utility/matrix_layout_policy.py`** (puro, senza Qt): `LARGE_MATRIX_EDGES = 600`, `LARGE_GRAPH_ATTRS`, `is_large_matrix(n_edges)`, `apply_large_graph_policy(graph_attr, n_edges)` (`splines=ortho → polyline`, `nodesep=0.3`, `ranksep=1`), `laid_out_size_points(dot_text)`, `safe_raster_dpi(dot_text, requested_dpi, max_px=32000, min_dpi=12)`, `set_dot_dpi(dot_text, dpi)`. In **`modules/utility/pyarchinit_matrix_exp.py`**: `_clamp_raster_dpi(dot_path, requested_dpi) -> (dpi, clamped)`, `_render_vector_copies(dot_path, formats=('svg', 'pdf'))`, `_large_matrix_notice(dpi)`, cablati in `HarrisMatrix.export_matrix`, `HarrisMatrix.export_matrix_2`, `ViewHarrisMatrix.export_matrix`, `ViewHarrisMatrix.export_matrix_3`. Misure: layout ortho 1 s a 400 archi, 12 s a 800, >25 min a 2039; polyline 0.5 s; bb 79 873 pt → JPG da 0 byte a 300 dpi (limite cairo 32 767 px). Test `tests/utility/test_matrix_layout_policy.py` (5).
- Docs: `dev_logs/CHANGELOG.md` bilingue; tutorial 03 e 11 in 10 lingue.

### English

**Large-DB fixes: node_uuid backfill lock, US sheet PDF with hundreds of relationships, Harris matrix above 600 edges** — pyarchinit tag `large-relations-5.13.12-alpha` (branch `Stratigraph_00001`, commit `8ef4347c`); the PDF and matrix fixes also shipped on `master` as **4.9.11** (tag `v4.9.11`, commit `26adb90b`). All found on an 11 MB SQLite DB with 1311 SU and two hubs with ~545 relationships each.

- **`scripts/migrations/_2026_05_node_uuid_backfill_lib.py`**: `backfill_uuids()` reflected the PKs with `inspect(handle.engine)` (a second pooled connection) inside `engine.begin()`; once the SQLite page cache spilled the writer escalated to an EXCLUSIVE lock and the reflection waited on the same transaction → `OperationalError: database is locked`. Now `inspect(conn)` on the transaction connection; `add_columns()` calls `_columns_of()` before opening the transaction. Test `tests/migrations/test_node_uuid_backfill.py::test_backfill_survives_page_cache_spill` (`PRAGMA cache_size=1`).
- **`modules/utility/pyarchinit_exp_USsheet_pdf.py`**: new constants `MAX_INLINE_RAPPORTI = 80`, `MAX_INLINE_RAPPORTI_INDEX = 40`, `ANNEX_TARGETS_PER_ROW = 60`, `_RAPPORTI_I18N` dictionary (it/en/de/fr/es); helper `_cap_rapporti(obj, limit, more_fmt)`; methods `single_US_pdf_sheet.cap_rapporti_for_sheet(lang)` (called after `unzip_rapporti_stratigrafici*()` in `create_sheet_archeo3_usm_fields_2` / `create_sheet_en` / `create_sheet_de`) and `single_US_pdf_sheet.create_rapporti_annex(lang) -> list[Flowable]` (annex with one row per 60 SU, `[]` when nothing was capped); every `build_US_sheets*` appends the annex after the sheet; `US_index_pdf_sheet.getTable*` cap at 40. Fixes `reportlab.platypus.doctemplate.LayoutError: Flowable ... too large on page` (rows 13-22 locked by SPANs; `Table(splitInRow=1)` rejected: one line per page). Tests `tests/utility/test_us_pdf_rapporti_overflow.py` (11).
- **New module `modules/utility/matrix_layout_policy.py`** (pure, no Qt): `LARGE_MATRIX_EDGES = 600`, `LARGE_GRAPH_ATTRS`, `is_large_matrix(n_edges)`, `apply_large_graph_policy(graph_attr, n_edges)` (`splines=ortho → polyline`, `nodesep=0.3`, `ranksep=1`), `laid_out_size_points(dot_text)`, `safe_raster_dpi(dot_text, requested_dpi, max_px=32000, min_dpi=12)`, `set_dot_dpi(dot_text, dpi)`. In **`modules/utility/pyarchinit_matrix_exp.py`**: `_clamp_raster_dpi(dot_path, requested_dpi) -> (dpi, clamped)`, `_render_vector_copies(dot_path, formats=('svg', 'pdf'))`, `_large_matrix_notice(dpi)`, wired into `HarrisMatrix.export_matrix`, `HarrisMatrix.export_matrix_2`, `ViewHarrisMatrix.export_matrix`, `ViewHarrisMatrix.export_matrix_3`. Measurements: ortho layout 1 s at 400 edges, 12 s at 800, >25 min at 2039; polyline 0.5 s; bb 79 873 pt → 0-byte JPG at 300 dpi (cairo limit 32 767 px). Tests `tests/utility/test_matrix_layout_policy.py` (5).
- Docs: bilingual `dev_logs/CHANGELOG.md`; tutorials 03 and 11 in 10 languages.
